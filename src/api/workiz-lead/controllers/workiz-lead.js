...
const staticPriceMap = {
  // tv-size
  under31: { price: 59, label: 'TV Size: Under 31"', workizId: 1003 },
  upTo31: { price: 59, label: 'TV Size: Under 31"', workizId: 1003 },
  "32-59": { price: 125, label: 'TV Size: 32"-59"', workizId: 1004 },
  "60-69": { price: 144, label: 'TV Size: 60"-69"', workizId: 1005 },
  "70-85": { price: 149, label: 'TV Size: 70"-85"', workizId: 1006 },
  "over-86": { price: 189, label: 'TV Size: 98"-100"', workizId: 1007 },
  ...
  corner: { price: 69, label: "Mount Type: Corner Mount", workizId: 1015 },
  ceilingMount: { price: 69, label: "Mount Type: Ceiling Mount", workizId: 1036 },
  ultraSlim: { price: 79, label: "Mount Type: Ultra Slim 0.3", workizId: 1040 },
  needHelp: { price: 0, label: "Mount Type: Need help choosing", workizId: 1013 },

  // wall
  drywall: { price: 0, label: "Wall Type: Drywall", workizId: 1017 },
  stoneBrickConcrete: {
    price: 49,
    label: "Wall Type: Brick / Stone / Concrete",
    workizId: 1018,
  },
  ceiling: { price: 39, label: "Wall Type: Ceiling", workizId: 1044 },
  tile: { price: 69, label: "Wall Type: Tile", workizId: 1043 },
  metalStuds: { price: 30, label: "Wall Type: Metal Studs", workizId: 1042 },
  notSure_wallType: { price: 0, label: "Wall Type: Not Sure", workizId: 1017 },

  // fireplace
  fireplace_yes: { price: 32, label: "Above Fireplace: Yes", workizId: 1045 },
  fireplaceYes: { price: 32, label: "Above Fireplace: Yes", workizId: 1045 },
  fireplace_no: { price: 0, label: "Above Fireplace: No", workizId: 1017 },

  // wires
  open: { price: 0, label: "Wire Concealment: Exposed", workizId: 1017 },
  cableChannelDrywall: { price: 43, label: "Wire Concealment: Cable Channel", workizId: 1047 },
  wallDrywall: { price: 93, label: "Wire Concealment: In-Wall", workizId: 1048 },
  socketDrywall: { price: 129, label: "Wire Concealment: In-Wall with Socket", workizId: 1049 },
  wallFireplace: {
    price: 109,
    label: "Wire Concealment: In-Wall (Fireplace)",
    workizId: 1051,
  },
  socketFireplace: {
    price: 149,
    label: "Wire Concealment: In-Wall with Socket (Fireplace)",
    workizId: 1052,
  },
  cableChannelBrick: {
    price: 52,
    label: "Wire Concealment: Cable Channel (Brick)",
    workizId: 1053,
  },
  wallBrick: {
    price: 249,
    label: "Wire Concealment: In-Wall (Brick)",
    workizId: 1054,
  },
  socketBrick: {
    price: 289,
    label: "Wire Concealment: In-Wall with Socket (Brick)",
    workizId: 1055,
  },
  wires_yes: { price: 109, label: "Wire Concealment: Yes", workizId: 1051 },
  wires_no: { price: 0, label: "Wire Concealment: No", workizId: 1017 },
  wires_notSure: { price: 0, label: "Wire Concealment: Not Sure", workizId: 1017 },
...
async function sendToProsBuddy(
  firstName,
  lastName,
  email,
  phone,
  ...
  try {
    ...
    const response = await axios.post(
      "https://dev.app.prosbuddy.ai/api/v1/webhook/ghl/create-lead",
      prosbuddyData,
    );
    ...
    strapi.log.error(
      "Error sending data to ProsBuddy:",
    ...
    try {
      if (!apiToken || !authSecret) {
        return ctx.internalServerError(
          "Workiz API credentials are not set in environment variables.",
        );
      }
      const {
        phone,
      ...
      try {
        ...
        strapi.log.error(
          "ProsBuddy API call failed, but continuing with Workiz flow:",
          prosbuddyError.message,
        );
      ...
      ctx.internalServerError(
        "An error occurred while sending the lead to Workiz.",
        { error: error },
      );
    }
  },
    ...
    try {
      ...
      if (
        ...
        return ctx.badRequest(
          "Missing contactInfo (name or phone) in request body",
        );
      }
      const {
        contactInfo,
      ...
      if (tvSizeStep.tvSelection) {
        const key =
          tvSizeStep.tvSelection === "notSure"
            ? "notSure_tvSelection"
            : tvSizeStep.tvSelection;
        valueCountPairs.push({ key, value: tvSizeStep.tvSelection, count: 1 });
      }
      if (tvSizeStep.extraTechnicans) {
        valueCountPairs.push({
          key: tvSizeStep.extraTechnicans,
          value: tvSizeStep.extraTechnicans,
          count: 1,
        });
      }
      ...
      if (mountingStep.mountType) {
        valueCountPairs.push({
          key: mountingStep.mountType,
          value: mountingStep.mountType,
          count: 1,
        });
      }
      ...
      if (wallStep.wallTypeProjector) {
        valueCountPairs.push({
          key: wallStep.wallTypeProjector,
          value: wallStep.wallTypeProjector,
          count: 1,
        });
      }
      if (wallStep.fireplace) {
        const key =
          wallStep.fireplace === "yes"
            ? "fireplace_yes"
            : wallStep.fireplace === "no"
              ? "fireplace_no"
              : wallStep.fireplace;
      ...
      if (fireplaceStep.fireplace) {
        const key =
          fireplaceStep.fireplace === "yes"
            ? "fireplace_yes"
            : fireplaceStep.fireplace === "no"
              ? "fireplace_no"
              : fireplaceStep.fireplace;
        // Avoid duplicate if already parsed from old structure
        if (!valueCountPairs.some((p) => p.key === key)) {
          valueCountPairs.push({
            key,
            value: fireplaceStep.fireplace,
            count: 1,
          });
        }
      }
      ...
      const additionalServicesStep =
        rest["additionalServices"] || rest["additional-services"] || {};
      if (
        additionalServicesStep &&
        typeof additionalServicesStep === "object"
      ) {
        ...
        ["soundbar", "soundbarMount", "screenInstallation"].forEach((field) => {
          if (additionalServicesStep[field]) {
            valueCountPairs.push({
              key: additionalServicesStep[field],
              value: additionalServicesStep[field],
              count: 1,
            });
          }
        });
        ...
        Object.keys(additionalServicesStep).forEach((key) => {
          ...
          if (
            ...
            typeof item === "object" &&
            item.value &&
            !["soundbar", "soundbarMount", "screenInstallation"].includes(key)
          ) {
            valueCountPairs.push({
              key: item.value,
              value: item.value,
      ...
      let priceMapItems = [];
      try {
        priceMapItems = await strapi.db
          .query("api::price-map.price-map")
          .findMany();
      } catch (err) {
        strapi.log.error(
          "Could not fetch price-map from database, using static fallback:",
          err.message,
        );
      }
      ...
      valueCountPairs.forEach(({ key, value, count }) => {
        ...
        if (resolvedItem && resolvedItem.workizId) {
          ...
          const tvSelection = tvSizeStep.tvSelection;
          let itemPrice = resolvedItem.price || 0;

          // Dynamic mounting cost adjustment based on tvSelection
          if (
            ["fixed", "tilting", "fullMotion", "ceilingMount"].includes(
              itemKey,
            ) &&
            tvSelection
          ) {
            if (tvSelection === "over-86") {
              if (itemKey === "fixed") itemPrice = 72;
              else if (itemKey === "tilting") itemPrice = 89;
              else if (itemKey === "fullMotion") itemPrice = 199;
              else if (itemKey === "ceilingMount") itemPrice = 99;
            } else if (tvSelection === "70-85") {
              if (itemKey === "fullMotion") itemPrice = 79;
            }
          }
      ...
      ctx.internalServerError(
        "An error occurred while sending the bestQuote lead to Workiz.",
        { error: error.message },
      );
    }
  },
